...
- **作業ブランチ**:
  ...
  - `release/v{X.Y.Z}` — リリース準備（`develop` から分岐 → `main` にスカッシュマージ → `main` を `develop` に反映）
...
- **PRのbaseブランチ**: 通常は `develop`、release は `main`、hotfix は `main`
- **マージ方式**: feature/bugfix → develop は通常マージ（`--merge`）、release → main は squash マージ（`--squash`）、リリース後に main → develop へ差分反映。詳細は `docs/specs/git-flow.md` の「マージ方式」セクション参照
