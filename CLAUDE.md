...
- 「コードレビューして」「セルフレビュー」 → `/code-review`
- 「ドキュメントレビューして」「仕様書レビュー」 → `/doc-review`
- 「コミットしてPR作成して」「ファイナライズして」 → `/auto-finalize`
- 「引き継ぎお願い」「セッション終了」 → `/handoff`
- 「前回の続き」「復帰して」 → `/restore`
- 「自動マージレビューチェックして」「全PRチェック」 → `/check-review-batch`
- 「仕様書を作って」「スペック作成」 → `/doc-gen`
- 「仕様書を更新して」「ドキュメント修正して」 → `/doc-edit`
