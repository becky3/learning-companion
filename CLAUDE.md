...
- **作業ブランチ**:
  ...
  - `release/v{X}.{Y}.{Z}` — リリース準備（`develop` から分岐 → `main` に squash マージ → `main` を `develop` に反映）
...
- **PRのbaseブランチ**: 通常は `develop`、release/hotfix は `main`
- **マージ方式**: feature/bugfix → develop は通常マージ（`--merge`）、release → main は squash マージ（`--squash`）、main マージ後は `main` → `develop` に通常マージで差分反映。詳細は `docs/specs/git-flow.md` の「マージ方式」セクション参照
