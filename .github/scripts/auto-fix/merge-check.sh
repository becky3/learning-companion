...
#   EXCLUDE_CHECK   — 任意: CI判定から除外するチェック名（自ワークフロー除外用）
#
# 出力（$GITHUB_OUTPUT）:
...
set -euo pipefail
# _common.sh is dynamically resolved at runtime
...
# jq query uses --arg for variable substitution, not shell expansion
