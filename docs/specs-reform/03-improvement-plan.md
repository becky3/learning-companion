...
<<未決（Phase 1）: 具体的なカテゴリ分類（カテゴリ名・各仕様書の所属）。各仕様書を実際に見ながら決定する>>
...
| ソースコード | `src/` 配下 (約20ファイル) | 約30箇所 | docstring のパス参照 |
| MCPサーバー | `mcp_servers/` 配下 (約20ファイル) | 約45箇所 | docstring のパス参照 |
| テスト | `tests/` 配下 (約34ファイル) | 約420箇所 | `test_ac{N}_` テスト名 |
...
**テスト名リネームの規模**: 約34ファイル・約420箇所のテスト関数名を変更する必要がある。仕様書ごとにマッピングテーブルを作成し、スクリプトで一括変換するアプローチを推奨する。
...
<<未決（Phase 4）: mixed-genius.md のプレースホルダー解決方法（実値の確定・テンプレート化等）>>
...
- CLAUDE.md の仕様書パス参照を新ディレクトリ構造に更新
...
| 5. 過去仕様の残存 | 1.3 記述ルール（過去情報禁止）+ 3. リファクタリング + 4. 陳腐化文書整理 | Phase 3, 4 |
...
| 9. その他 | 4. 陳腐化文書整理 + 5. overview.md整合性 + 1.2 許容/禁止基準の明文化 | Phase 1, 4, 5 |
