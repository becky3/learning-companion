...
## E2E テスト — 全通しまでの道のり（Issue #351, #386, #394, #398, #407）

### 何が起きたか

sleep ポーリング方式の実装完了後、auto-implement パイプラインの E2E テストを実施した。初回テスト（Issue #389 → PR #392）で前半（ポーリング〜Copilot 検知〜ラベル付与）は正常動作を確認したが、後半（指摘修正〜自動マージ）は複数の問題で到達できなかった。

最終的に PR #409（Issue #343）で**パイプライン全ステップの初の完全通過**を達成するまでに、4つの問題を順に解決する必要があった。

### 解決した問題と経緯

| # | 問題 | 原因 | 対応 | PR |
|---|------|------|------|-----|
| 1 | 禁止パターンで大半の Issue がブロック | CLAUDE.md が禁止対象で、ほぼ全ての auto-implement PR が停止 | CLAUDE.md, .claude/settings.json を禁止パターンから削除 | #395 |
| 2 | private リポジトリ権限エラー | `statusCheckRollup`, `checkSuite.workflowRun` が private リポジトリで権限不足 | `checks: read`, `actions: read` を permissions に追加 | #399, #401 |
| 3 | REPO_OWNER_PAT 不要な変更（誤修正） | CI wait の「no checks reported」を GITHUB_TOKEN の制約と誤認 | REPO_OWNER_PAT に変更したが根本原因ではなかった。後に問題4で真因判明 | #404 |
| 4 | CI wait で永久リトライ（「no checks reported」） | 存在しない外部 CI ワークフローを `gh pr checks` で待機する設計 | CI wait ステップ自体を削除。品質チェックはエージェント内テストで担保 | #408 |

### 問題3: 推測による誤修正の教訓

「no checks reported」エラーに対し、以下の推測チェーンで誤った修正を実施した。

```
「no checks reported」→「GITHUB_TOKEN ではチェックが見えないのでは」→「REPO_OWNER_PAT に変更すれば見えるはず」
```

ユーザーに以下を指摘された。

- 「推測で判断してない？」
- 「なんで存在しない workflow を使おうとしてる？」
- 「実装後のエージェントによるチェックと外部CIを混同されてない？」

**真因**: そもそも外部 CI ワークフロー（pytest/ruff/mypy を実行する別の GitHub Actions ワークフロー）が存在しない。品質チェックは claude-code-action 内のエージェントが実行しており、`gh pr checks` で確認できる外部チェックではない。CI wait ステップの設計自体が「存在しない外部CI」を前提としていた。

これは CLAUDE.md の「判断の裏付け義務」に対する明確な違反だった。外部 CI ワークフローが存在するかどうかをワークフロー定義で確認すれば一発で判明する事実を、推測で飛ばしてしまった。

### 最終結果: PR #409 での全通し

問題4（CI wait 削除、PR #408）のマージ後、Issue #343 で再テストを実施。全ステップが初めて最後まで通った。

```
Issue #343 に auto-implement ラベル付与
  → claude.yml が起動、PR #409 を自動作成
  → copilot-auto-fix.yml が起動
    → Copilot レビュー検知（sleep ポーリング、30秒間隔）
    → auto:copilot-reviewed ラベル付与
    → 指摘検出（AC7 記述不整合）
    → claude-code-action が /check-pr で自動修正
    → 対応コメント投稿 + スレッド resolve
    → マージ判定（5条件全クリア）
    → 自動マージ（auto:merged ラベル付与）
  → post-merge.yml が起動
    → Issue #375 にマージ記録コメント追加
    → 次 Issue 候補の提示
  → Issue #343 自動クローズ
```
...
#### 1. リポジトリ private 化の即断

auto-implement のセキュリティリスク（Issue poisoning、フォーク PR 注入）に対し、ユーザーが「後悔先に立たず」で即座に private 化を決断。ポートフォリオの懸念より、セキュリティを気にして実験が萎縮することの方がコストが高いという判断。これにより禁止パターンの緩和も安心して行えた。

#### 2. merge-check.sh の堅牢な設計

`statusCheckRollup` で `no_checks`（チェック未設定）を自動 PASS する設計が最初から入っていた。CI wait ステップを削除してもマージ判定は正しく動作した。設計時の防御的コーディングが救いになった。

#### 3. doc-reviewer による更新漏れ検出

CI wait 削除に伴う仕様書修正で、初回は直接変更した4箇所のみ修正したが、doc-reviewer が同一ファイル内の残り6箇所（概要フロー、Resolve タイミング、AC 4件）の旧記述を検出した。

### ハマったこと・改善点

#### 1. 裏付けなしの推測チェーン（問題3）

最も大きな失敗。「no checks reported」→ 推測 → REPO_OWNER_PAT 変更という流れで、根本原因（外部CI不存在）を見落とした。CLAUDE.md の「判断の裏付け義務」が機能しなかった。

**教訓**: エラーメッセージに対して「なぜそうなるか」を推測で埋めるのではなく、前提条件（この場合は「外部CIワークフローは存在するか？」）を事実確認してから対策を打つ。

#### 2. 仕様書と実態の乖離を見落とした

仕様書（copilot-auto-fix.md, auto-progress.md）に「CI完了待機」「gh pr checks でポーリング」と明記されていたが、対応する外部CIワークフローは一度も作成されていなかった。仕様書を書いた時点で「品質チェックはエージェント内テストで行う」という実態との乖離に気づくべきだった。

#### 3. private リポジトリでの暗黙の権限

public リポジトリでは暗黙的に許可されていた `checks: read` と `actions: read` が、private 化で明示的宣言が必要になった。GitHub の権限モデルの public/private 差異を事前に調査していなかった。

#### 4. セッション途中のルールマージの拾い漏れ

PR #397（@メンション回避ルール）がセッション途中にマージされたが、コンテキストに反映されず、PR #404 のコメントで `@copilot-pull-request-reviewer` と書いて不要な PR #405 が作成された。

### 次に活かすこと

1. **推測で対策を打つ前に前提条件を事実確認する**: 「〜のはず」「〜だろう」で判断を確定させない。特に外部サービス（GitHub API、CI/CD）の挙動は推測が外れやすい。`gh` コマンドや設定ファイルで確認すれば一発で分かることが多い

2. **仕様書を書いたら実態との整合性を確認する**: 仕様書に書いた前提（外部CIの存在等）が実際の環境で成立しているか、実装前に検証する。机上の設計と実環境の乖離は E2E テストで初めて発覚することが多い

3. **private リポジトリ化時は権限を棚卸する**: public → private の移行時に、暗黙的に許可されていた権限が明示的宣言を要求するようになる。全ワークフローの permissions セクションを確認する

4. **E2E テストは設計の「嘘」を暴く最良の手段**: 机上の設計レビューでは見つからない問題（存在しない外部CI、private 権限、ラベル未作成等）が E2E テストで次々に発覚した。パイプライン設計は、できるだけ早い段階で実環境で通すべき

5. **削除は追加より価値が高い**: CI wait ステップの削除（121行減）でパイプラインが大幅にシンプルになった。「存在しないものを待つ」コードは複雑さだけを増やし価値を生まない
...
- 関連Issue:
  ...
  #386（自動レビュー最適化）, #394（禁止パターン緩和）, #398（private リポジトリ権限修正）,
  #407（CI 待機ステップ削除）, #351（パイプライン全体の収束問題 — 親 Issue）
