...
機能追加・変更・削除を行った際に、README.md への反映漏れがないかを検出する。

**チェック対象セクション:**

| セクション | チェック条件 |
|-----------|------------|
| **主な機能** | 新機能の追加・既存機能の説明変更・機能の削除 |
| **技術スタック** | 新しいライブラリ・フレームワークの追加（`pyproject.toml` の依存関係変更時） |
| **主な環境変数** | 新しい環境変数の追加・既存の環境変数の変更（`settings.py` / `.env.example` の変更時） |
| **プロジェクト構造** | 新しいディレクトリ・主要ファイルの追加（`src/` 配下の構造変更時） |
| **ドキュメント > 機能仕様** | 新しい仕様書（`docs/specs/f*.md`）の追加時 |
| **ドキュメント > Claude Code 関連** | Claude Code 関連の仕様書（`docs/specs/` 配下）の追加時 |

**検出ロジック:**

変更差分に以下のパターンが含まれるが、README.md の変更が含まれない場合に指摘する:

1. `src/services/` に新しいサービスファイルが追加された → 「主な機能」セクションの更新が必要か確認
2. `pyproject.toml` の `[project.dependencies]` に変更がある → 「技術スタック」セクションの更新が必要か確認
3. `src/config/settings.py` に新しい設定項目が追加された、または `.env.example` に変更がある → 「主な環境変数」セクションの更新が必要か確認
4. `src/` 配下に新しいディレクトリ・モジュールが追加された → 「プロジェクト構造」セクションの更新が必要か確認
5. `docs/specs/` に新しい仕様書が追加された → 「ドキュメント」セクションの更新が必要か確認

**適用条件:**

- 変更差分のレビュー時のみ適用（特定ファイル指定時は適用しない）
- README.md 自体が変更差分に含まれている場合は、内容の整合性を確認する
- テストのみの変更、リファクタリング（外部仕様に影響しない変更）、CI/CD 変更ではスキップする

**優先度:**

- 新機能追加時の「主な機能」セクション未更新 → **Warning**
- 新しい環境変数追加時の「主な環境変数」セクション未更新 → **Warning**
- その他のセクション未更新 → **Suggestion**
...
Warning / Suggestion レベルの指摘であっても、「スコープ外」「別 Issue に切り出す」「このままで可」と判断する前に、以下の調査を実施すること。
...
| 調査結果 | 判断 |
|---------|------|
| 修正コストが低い（数行〜十数行の変更） **かつ** 放置するとリスクがある | **スコープ内として修正を推奨** |
| 修正コストが高い（大規模リファクタリングが必要） | スコープ外として Issue 切り出しを推奨（調査結果を添えて報告） |
...
- Warning / Suggestion を一律で「このままで可」とすること
...
- [ ] AC20: 機能追加・変更時に README.md の更新漏れを検出できる
