...
2. **変更ファイルの分類**
   - `src/**/*.py` → 対応テストを推定、ruff・mypyの対象
   - `tests/*.py` → そのまま実行対象に追加、ruffの対象
   - `pyproject.toml`, `conftest.py` → 全テスト実行にフォールバック（設定変更とみなし安全側に倒す）
   - `*.md`（`docs/**/*.md`, `CLAUDE.md`, `.claude/**/*.md` 等） → markdownlintの対象（pytest・ruff・mypyはスキップ）
   - その他（画像、設定ファイル等） → テスト対象外（diffモードではすべてのチェックをスキップ）
