...
Learning Companionプロジェクトは仕様駆動開発を採用しており、各機能の受け入れ条件（AC）に対応するテストが `tests/` ディレクトリに配置されている。また、コーディング規約として `ruff` によるlintと `mypy` による型チェックの使用が `CLAUDE.md` で規定されている。
...
**注**: ruff・mypyのいずれかが失敗してもプロセスを中断せず、すべてのチェックを実行して統合レポートを生成する。
