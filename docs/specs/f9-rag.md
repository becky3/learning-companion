...
**similarity_threshold 設定時のBM25-onlyドキュメント除外**:

similarity_threshold が設定されている場合、ベクトル検索の品質ゲートを通過しなかったドキュメントは、BM25ヒットの有無にかかわらず最終結果から除外される。BM25は品質ゲート通過済みドキュメントのスコアを「ブースト」する役割に限定され、ゲート未通過ドキュメントの「レスキュー」はしない。

| ベクトル結果 | BM25ヒット | threshold設定 | 最終結果 |
|-------------|-----------|--------------|---------|
| 閾値通過 | あり | あり | 返却（CC統合スコア） |
| 閾値通過 | なし | あり | 返却（ベクトルスコアのみ） |
| 閾値未通過 | あり | あり | **除外** |
| 閾値未通過 | なし | あり | **除外** |
| なし（BM25-only） | あり | あり | **除外** |
| 閾値通過 | あり | None | 返却（CC統合スコア） |
| 閾値通過 | なし | None | 返却（ベクトルスコアのみ） |
| 閾値未通過 | あり | None | 返却（BM25レスキュー） |
| 閾値未通過 | なし | None | 返却（低スコア） |
| なし（BM25-only） | あり | None | 返却（BM25スコアのみ） |

キーワード検索を優先したい場合は `similarity_threshold=None` に設定するか、閾値を緩和する。
...
- [ ] **AC82**: similarity_threshold 設定時、ベクトル検索で閾値を通過しなかったドキュメントが BM25 ヒットのみで最終結果に含まれないこと
...
| 日付 | 内容 |
|------|------|
| 2026-02-18 | similarity_threshold設定時のBM25-onlyドキュメント除外ルールを追加、AC82追加（#499） |
