...
- `@bot feed add <URL> [カテゴリ]` — フィード追加（タイトル自動取得、カテゴリ省略時は「一般」、複数URL対応）
...
- RSSフィードをfeedparserで取得し、`feed.title` からフィード名を自動設定する
- タイトルが取得できない場合（パースエラー、titleタグ未設定等）はURLをフォールバックとして使用する
- カテゴリは省略時「一般」固定（LLM推定は行わない）
...
- [ ] AC4: Slackメンション+キーワード（`deliver`）をトリガとして収集・配信ジョブを実行できる（APSchedulerは廃止し、定期実行はSlackリマインダーで運用するが、その場合もリマインダー本文にボットメンション＋キーワード（例: `@Daisy deliver`）を含めること）
...
- [ ] AC7: フィードの追加・削除・有効/無効切替ができる
  - [ ] AC7.1: フィード追加（タイトル自動取得、カテゴリ指定あり、省略時は「一般」）
  - [ ] AC7.2: 複数フィード一括追加
  ...
  - [ ] AC7.14: feed addでRSSフィードのtitleタグからフィード名を自動取得する（取得失敗時はURLをフォールバック）
...
| `src/scheduler/jobs.py` | 収集・配信ジョブ（ハンドラから呼び出し） |
