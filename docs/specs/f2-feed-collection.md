...
**処理:**
1. フィードごとに記事をグループ化（フィードURL降順）
2. フィードごとに親メッセージ＋スレッド形式でSlackに逐次投稿
...
```
- 各記事の全内容（更新日時・タイトル・画像・要約）をスレッド内に投稿
- **1記事につき1つのSlackメッセージ**として投稿（一括ではない）
- 投稿は**逐次型**（配信準備が整った記事から順次投稿）
...
**処理:**
1. 「テスト配信を開始します...」と応答
2. 有効フィードをID昇順で最大5件取得
3. 各フィードの全記事を取得（`delivered` フラグに関わらず）
4. 親メッセージ＋スレッド形式で投稿（通常配信と同じ形式）
5. `delivered` フラグは **更新しない**（副作用なし）
6. ヘッダーは `:test_tube: フィードテスト配信` に変更
...
- [ ] AC14: フィード毎に個別メッセージとして配信される（カテゴリ別ではない）
  ...
  - [ ] AC14.2: 記事の全内容（タイトル・画像・要約）はスレッド内に1記事1投稿で投稿される
...
- [ ] AC15: `feed test` コマンドでテスト配信ができる
  ...
  - [ ] AC15.3: 上から5フィード分のみ対象とする
  - [ ] AC15.4: delivered フラグを更新しない（副作用なし）
  - [ ] AC15.5: 通常配信と同じ親メッセージ+スレッド形式で出力する
