...
ジャーナルやMEMORY.mdから学びトピックを自動抽出し、Zenn形式の技術記事を生成するClaude Codeスキル。

## 背景

開発中に得た知見（ハマったこと、解決策、教訓）はジャーナルに記録されているが、これを対外発信できる形式（Zenn記事）に変換するには手間がかかる。自動抽出・記事生成を仕組み化することで:

- ジャーナルに蓄積された学びを効率的に記事化
- 発信のハードルを下げる（「見て選ぶだけ」）
- 記事化履歴を参照できる

## ユーザーストーリー

- 開発者として、`/topic` でジャーナルから記事化候補を自動抽出・表示したい
...
- (2026-02-15) RSS日付パースの罠: memory/journal/20260215-xxx.md
```
...
| 状況 | 使用する表現 |
|------|-------------|
| 画面UI・実行結果・エラー画面など | `{{画像の説明}}` プレースホルダー |
| DB関係図・シーケンス図・フローチャートなど | mermaid コードブロック（従来通り） |

- 画像プレースホルダーは、ユーザーが後から実際のスクリーンショットや画像に差し替えることを前提とする
- mermaid で表現できる技術的な図表（フローチャート、シーケンス図、ER図等）は、プレースホルダーではなく mermaid を使用する
...
- [ ] AC8: スクリーンショット・挿絵が効果的な箇所に `{{画像の説明}}` プレースホルダーが挿入される
- [ ] AC9: DB関係図・フローチャート等の技術的な図表には mermaid が使用される（プレースホルダーではなく）
...
- [ ] `/topic` でジャーナル・レトロから候補が抽出される（AC1-3）
