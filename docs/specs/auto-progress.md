...
| Require linear history | NO | 通常マージで開発履歴を保持（マージ方式は `git-flow.md` 参照） |
...
| Require linear history | NO | squash マージはリリース時に使用（マージ方式は `git-flow.md` 参照） |
...
マージ方式: `gh pr merge --merge`（通常マージ。マージ方式の詳細は `git-flow.md` の「マージ方式」セクション参照）
