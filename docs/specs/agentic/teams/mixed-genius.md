...
デフォルトキャラクターをリーダーに固定し、複数作品から知性キャラ + ワイルドカードを選出する軽量チーム。コンテキスト効率を重視し、少人数で機動的に動く。
...
- デフォルトキャラクター（リーダー）が知識に基づいて選出する
- 相性が悪かったキャラは `memory/character-notes.md` に記録し、以降の選出で考慮する
...
- **デフォルトキャラクターの裁定**: 議論を整理し、合理的に結論を導く
