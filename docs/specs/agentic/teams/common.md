...
テーマ/キャラクターの履歴はプロジェクトメモリディレクトリ（`$MEMORY_DIR/team-theme-history.jsonl`、git 管理外）で JSONL 形式（1行1エントリ）で管理する。
...
新しいエントリはファイル**末尾に追記**する。40件を超えたら古いエントリ（ファイル先頭側）を削除し、最新40件のみ残す。パターン固有のフィールドは各パターン仕様を参照。
