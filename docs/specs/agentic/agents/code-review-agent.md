...
| モード | 用途 | レビュー範囲 |
| --- | --- | --- |
| `diff` | 変更差分のレビュー（デフォルト） | 変更差分のみ |
| `full` | ファイル全体のレビュー | ファイル全体を全観点でチェック |

- `diff`: 明示的に「差分レビュー」を指定した場合、またはモード未指定時（デフォルト）
- `full`: 明示的に「フルレビュー」を指定した場合

## 処理フロー

1. **レビューモードの判定**: `full` 指定時はフルレビュー、それ以外は diff モード（デフォルト）
2. **レビュー対象の特定**: ファイル指定があればそれを、なければ `git diff --cached` → `git diff` の順でレビュー対象を決定
3. **仕様書の参照**: 本仕様書を読みレビュー基準を取得。対象ファイルに対応する仕様書があれば読み込み
4. **コードの読み込み**: テストファイルの場合は対応する実装ファイルも、実装ファイルの場合は対応するテストも確認
5. **観点ごとの検査**: 10 の観点に基づき問題を検出し、優先度を判定
6. **レポート生成**: ファイルごとに検出した問題を優先度別に整理。各問題にファイルパス・行番号・改善案を付与
