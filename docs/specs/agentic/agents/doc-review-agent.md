...
仕様書（`docs/specs/`）、README.md、および ARCHITECTURE.md の品質をレビューするサブエージェント。仕様駆動開発の観点から、構造の完全性、情報の過不足、コードとの整合性をチェックし、改善提案を行う。
...
### ARCHITECTURE.md 固有レビュー観点

| # | 観点 | 内容 |
| --- | --- | --- |
| 1 | ディレクトリ存在確認 | 記載されたディレクトリが実際に存在するか |
| 2 | 仕様書対応の整合性 | 対応表の仕様書ファイルが `docs/specs/` に存在するか |
| 3 | 図の正確性 | mermaid 図のモジュール名が実在するディレクトリと一致するか |

### レビューモード

| モード | 用途 | レビュー範囲 |
| --- | --- | --- |
| `diff` | 軽微な修正・レビュー指摘対応（デフォルト） | 変更差分のみを関連観点でチェック |
| `full` | 新規作成・大幅改訂 | ファイル全体を全観点でチェック |
...
1. **対象の特定**: ファイル指定があればそれを、なければ `docs/specs/` 全仕様書と README.md、ARCHITECTURE.md を対象
2. **ドキュメント種別の判定**: README.md / ARCHITECTURE.md / 仕様書で適用基準を切り替え
3. **構造チェック**: テンプレートのセクション有無（仕様書）/ 一般的な構成要素（README）
4. **内容の詳細レビュー**: 各セクションの品質評価、過不足検出。許容パターンは除外
5. **整合性確認**: 関連ファイル存在確認、docstring チェック（仕様書）/ リンク切れ・CLAUDE.md 一貫性（README）/ ディレクトリ存在・仕様書対応表・mermaid 図の正確性（ARCHITECTURE）
6. **レポート生成**: 優先度別に整理
