...
1. **実装との不整合の場合**: `Grep` / `Glob` ツールで実際のコード・ファイル構成を確認し、ドキュメントの記載が実態と合っているか検証する
2. **情報の過不足の場合**: 対象セクションが実装時に参照される頻度・重要性を確認し、実害があるか判断する
3. **構造上の問題の場合**: 修正に必要な変更量（数行の追記で済むか、大幅な構造変更が必要か）を確認する

#### 判断基準

| 調査結果 | 判断 |
|---------|------|
| 修正コストが低い（数行の追記・修正） **かつ** 放置すると実装時に混乱を招く | **スコープ内として修正を推奨** |
| 修正コストが高い（大幅な構造変更が必要） | スコープ外として Issue 切り出しを推奨（調査結果を添えて報告） |
| 調査の結果、実害がないと判明 | 「調査した結果、現時点では実害なし。理由: ...」と報告 |

#### 禁止パターン

- 影響範囲を調査せずに「スコープ外」と判断すること
- 「ドキュメントの話なので後回し」と調査結果なしで断定すること
- Warning / Suggestion を一律で「このままで可」とすること

### 許容パターン

<!-- 移行予定（#625）: エージェント/スキル開発ガイドラインに移行する -->
...
- [ ] AC9: テンプレート準拠チェックで一貫性のない構造を指摘できる
...
- [ ] AC19: 既存の仕様書レビュー機能（AC1〜AC9）は引き続き動作する（後方互換性）
