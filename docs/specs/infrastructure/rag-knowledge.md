...
| rag_crawl_preview | URL、パターン | クロール対象ページの一覧をプレビューする（取り込みは行わない） |
...
| `rag preview <URL> [パターン]` | rag_crawl_preview |
| `rag add <URL>` | rag_add |
...
指定 URL のリンク集ページから、クロール対象となるページの一覧をプレビューする機能。
実際の取り込み（チャンキング・Embedding 生成・DB 格納）は行わず、
どのページが取得対象になるかを事前に確認できる。

**用途**:

- クロール前の対象ページ確認（意図しないページの混入防止）
- URL パターン（正規表現）の動作検証
- robots.txt によるフィルタリング結果の確認

**振る舞い**:

1. リンク集ページからリンクを抽出する（既存の `crawl_index_page` と同じロジック）
2. 抽出した各 URL のページタイトルを取得する
3. URL とタイトルの一覧を返す

**タイトル取得**:

- 各ページに HTTP GET リクエストを送信し、HTML の `<title>` タグからタイトルを抽出する
- 取得時は既存のクローラーと同じ安全対策（SSRF 対策、robots.txt 遵守、レート制限）を適用する
- タイトル取得に失敗した場合は、タイトルを空文字とし、URL のみを返す（エラーで処理を中断しない）

**インターフェース**:

MCP ツール `rag_crawl_preview` の入出力:

| 項目 | 内容 |
| --- | --- |
| 入力 | URL（リンク集ページ）、パターン（正規表現、任意） |
| 出力 | ページ一覧テキスト（番号、タイトル、URL を含む） |
| 副作用 | なし（DB への書き込みは行わない） |

Slack コマンド `rag preview <URL> [パターン]`:

- 結果をスレッド内にテキスト形式で投稿する
- 取得件数が多い場合でも全件を表示する（上限は既存の最大クロールページ数設定に従う）

CLI サブコマンド `crawl-preview`:

| サブコマンド | 振る舞い |
| --- | --- |
| crawl-preview | 指定 URL のクロール対象ページ一覧を標準出力に表示する |
...
| --- | --- | --- |
| `--url` | 必須 | リンク集ページの URL |
| `--pattern` | 任意 | URL フィルタリング用の正規表現パターン |
| `--format` | 任意 | 出力形式（`text` または `json`） |
...
```mermaid
flowchart LR
    REQ["URL + パターン"] --> IDX["リンク集ページからURL抽出"]
    IDX --> FETCH["各ページのタイトル取得"]
    FETCH --> LIST["タイトル + URL 一覧を返却"]
```
...
| コンポーネント | 役割 |
| --- | --- |
| ナレッジサービス | 取り込み・検索・削除・プレビューのオーケストレーション |
...
| プレビュー時のタイトル取得失敗 | タイトルを空文字とし、URL のみを返す。エラーで処理を中断しない |
| プレビュー時にリンクが 0 件 | リンクが見つからなかった旨のメッセージを返す |
