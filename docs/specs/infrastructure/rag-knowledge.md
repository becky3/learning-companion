...
MCP サーバーとして外部プロセスで動作し、stdio トランスポートで接続する。
...
- ai-assistant からは MCP サーバー設定（`config/mcp_servers.json`）で外部リポの RAG サーバーを stdio 起動する
- デフォルト設定では rag-knowledge リポジトリが ai-assistant と同じ親ディレクトリにクローンされていることを前提とする（相対パス `../rag-knowledge`）。異なる配置の場合は `config/mcp_servers.json` のパスを環境に合わせて変更する
...
| 連携先 | 用途 | 接続方式 |
| --- | --- | --- |
| rag-knowledge リポジトリ | RAG MCP サーバー | stdio（`uv --directory` で外部リポを起動） |
