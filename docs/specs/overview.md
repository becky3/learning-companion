...
- **作業ブランチ**:
  ...
  - `release/v{X.Y.Z}` — リリース（`develop` → `main` にスカッシュマージ → `main` を `develop` に反映）
  - `hotfix/{修正内容}-#{Issue番号}` — 緊急修正（`main` → `main` + `develop`）
- コミット: `feat(f{N}): 説明 (#{Issue番号})`
- PR作成時に `Closes #{Issue番号}` でIssueを紐付け（feature/bugfix: base `develop`, release: base `main`, hotfix: base `main`）
- マイルストーンでStep単位の進捗管理
