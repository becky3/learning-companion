...
- **mermaid ブロック内の `#` 禁止**: GitHub の Mermaid レンダラーは `#` をパースできずエラーになる（CSS カラーコード `#xxx`/`#xxxxxx` は例外）。`#` が必要な箇所は文脈に応じて以下のように置換する:

  | 使用箇所 | 修正方法 | 例 |
  |----------|---------|-----|
  | ブランチ名中の Issue 番号 | `#` を `N` に置換 | `feature/chat-#1` → `feature/chat-N1` |
  | コメント行 (`%%`) | `#` を削除または言い換え | `%% see #123` → `%% see issue 123` |
  | ノード ID・ラベル | `#` を削除または別文字に | `node#1` → `node-1` |
  | commit id の文字列 | `#` を削除 | `commit id: "fix #5"` → `commit id: "fix 5"` |
...
テンプレート本体は dotfiles（`~/.claude/docs/templates/`）で管理する。各テンプレートは自己完結しており、ヘッダー形式・セクション定義はすべてテンプレート内に記載されている。
...
Issue の種類に応じたテンプレートを定義する。テンプレート本体は dotfiles（`~/.claude/docs/templates/issues/`）で管理する。
