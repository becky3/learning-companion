...
<!-- このセクションは #572 の移行作業が完了したら削除する -->

旧仕様書（`docs/specs/` 直下）を本スタイルガイドに準拠させる移行作業についての指示。

### 出力先

移行で作成する仕様書は `docs/specs-new/` を親ディレクトリとする。カテゴリ別サブディレクトリの構造はセクション1と同一:
...
移行完了後に `docs/specs/` と `docs/specs-new/` を入れ替える。
...
移行中は「関連ドキュメント」セクションのリンク先ファイル名が旧名称（f番号付き等）のままである。全ファイルの移行完了後に、リンクをまとめて新ファイル名に修正する（#628 で対応）。
