...
テンプレート本体は `.claude/docs/templates/` に配置する。各テンプレートは自己完結しており、ヘッダー形式・セクション定義はすべてテンプレート内に記載されている。
...
Issue の種類に応じたテンプレートを定義する。テンプレート本体は `.claude/docs/templates/issues/` に配置する。
