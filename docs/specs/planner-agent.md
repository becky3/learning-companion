...
**AC番号の付与ルール**: 出力フォーマットの2つのパターン（独立機能 / 既存ACサブ機能）に従い、計画対象に応じて番号体系を選択すること。既存仕様書の特定ACを詳細化する場合は、必ず `AC{親番号}.{連番}` を使用する。
...
エージェント定義 (`.claude/agents/planner.md`) の出力フォーマットに準拠する。詳細はエージェント定義の「出力フォーマット」セクションを参照。
