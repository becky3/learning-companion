...
- 実装タスク時は2名以上の実装担当を配置する（分量が少ない場合はリーダー判断で1名も可）
- 分量に応じて柔軟に分担し、各担当がコード・ドキュメントの両方を対応できるようにする
- **方針変更時**: リーダーは変更箇所を具体的に指示すること（ファイル名・行番号レベル）。複数ファイルにまたがる修正では、変更箇所の追跡コストが高くなるため
