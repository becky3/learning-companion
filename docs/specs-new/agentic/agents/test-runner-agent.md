...
テスト実行およびコード品質チェック（lint・型チェック・Markdown チェック）を自動化するサブエージェント。テストの実行、失敗時の分析、修正提案、再実行までを一貫して行う。
...
- コード品質チェック（ruff, mypy, markdownlint）
...
| `diff` | 変更ファイル関連のみ | 変更に関連するテストのみ |

- `full`: 明示的に「全テスト」を指定した場合、またはモード未指定時（デフォルト）
- `diff`: 明示的に「差分テスト」「diff テスト」を指定した場合
