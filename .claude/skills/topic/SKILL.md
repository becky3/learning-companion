...
Phase A からの番号返信、または `/topic <テーマ>` によるテーマ直接指定、いずれの場合も以下の流れで記事を生成する。

1. **素材の収集**
   - 番号選択の場合: Phase A で取得済みのスキャン結果からソースファイルの該当セクションを読み込み
...
- 1記事あたり3〜5箇所程度を目安に、再現可能なものだけ配置する（過去の状態で再現不能なスクショは入れない）
