...
pytest による自動テスト実行、ruff によるリント、mypy による型チェック、shellcheck によるシェルスクリプトチェックを実行し、結果を分析して修正提案を行う。

ドキュメント品質チェック（markdownlint・mermaid-lint）は共通スキル `/doc-check` が担当する。
...
- Markdown ファイル（`*.md`）のみの変更 → 本スキルのチェックはスキップ（`/doc-check` が対応）
- シェルスクリプト（`*.sh`）のみの変更 → pytest・ruff・mypy はスキップ、shellcheck のみ実行
...
**重要**: pytest・ruff・mypy・shellcheck のいずれかが失敗してもプロセスを中断せず、すべてのチェックを実行して統合レポートを生成する。
...
| `mcp_servers/rag/*.py` | `tests/test_rag_*.py` + RAG 精度テスト判定 |
| `mcp_servers/rag/embedding/*.py` | `tests/test_embedding.py` + RAG 精度テスト判定 |
...
RAG 関連ファイル（`mcp_servers/rag/**`, `mcp_servers/rag/embedding/**`）が変更された場合、通常のテストに加えて RAG 精度テストの実行を検討する。
...
| Embedding プロバイダー（`mcp_servers/rag/embedding/**`） | **必須** |
| 類似度閾値・検索パラメータ | **必須** |
